...
public class Tpa {
    Predicate<CommandSender> hasInvites = (sender) -> {
        if (!(sender instanceof Player)) return false;

        return PlayerManager.getPlayer(((Player) sender).getUniqueId()).hasIncoming();
    };

    Predicate<CommandSender> othersOnline = (sender) -> {
        if (!(sender instanceof Player)) return false;

        return Main.moreThanOnePlayer;
    };

    public Tpa() {
        new CommandAPICommand("tpa")
                .withRequirement(this.othersOnline)
        ...
        new CommandAPICommand("tpaccept")
                .withRequirement(this.hasInvites)
                .executesPlayer((p, args) -> {
                    if (p.getScoreboardTags().contains("inRace")) {
                        throw CommandAPI.failWithString("You can't use this command while in a race");
                    }
                    ...
                    for (UUID uuid : player.getIncoming()) {
                        ...
                        if (invited != null) {
                            invited.teleport(p);
                            PlayerManager.getPlayer(uuid).afterTeleport(p.getUniqueId());
